 ...
 * VERSION: 2025-09-23-v2 - Route fixes applied
...
app.use((req, res, next) => {
  ...
  // Special handling for API routes - force JSON response
  if (req.path.includes('/api/')) {
    res.setHeader('Content-Type', 'application/json');
    console.log('API route detected - setting JSON content type');
  }
...
app.get('/questionnaire/api/debug', (req, res) => {
  res.json({
    success: true,
    message: 'Debug route working - server updated successfully',
    timestamp: new Date().toISOString(),
